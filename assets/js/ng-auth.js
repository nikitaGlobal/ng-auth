  ...
  NG_Auth.Verification = {
    ...
    bindPhoneForm: function () {
      ...
      phoneInput.addEventListener("input", function () {
        ...
        let formatted = "+";
        if (val.length > 0)
          formatted += val.substring(0, Math.min(1, val.length));
        if (val.length > 1)
          formatted += " (" + val.substring(1, Math.min(4, val.length));
        if (val.length >= 4)
          formatted += ") " + val.substring(4, Math.min(7, val.length));
        if (val.length >= 7)
          formatted += "-" + val.substring(7, Math.min(9, val.length));
        ...
        self
          ...
          .done(function (data) {
            if (data.success) {
              ...
              if (data.code) {
                var codeInput = document.getElementById("ng-auth-code");
